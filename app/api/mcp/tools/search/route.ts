...
export async function POST(request: NextRequest) {
  ...
  if (authError) {
    mcpLogger.logToolInvocation(TOOL_NAME, '/api/mcp/tools/search', 'POST', false, Date.now() - start, authError.status || 401, ua, undefined, 'Authorization failed (missing scope posts:read)')
    return authError
  }
