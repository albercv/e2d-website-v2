...
export function Orb({ hue = 202, hoverIntensity = 3.2, rotateOnHover = true, forceHoverState }: OrbProps) {
  ...
  useEffect(() => {
    const container = ctnDom.current;
    if (!container || !rendererRef.current || !programRef.current || !meshRef.current) return;
    ...
    const isControlled = forceHoverState !== undefined;
    ...
    const update = (t: number) => {
      ...
      const effectiveHover = isControlled ? (forceHoverState ? 1 : 0) : targetHover;
